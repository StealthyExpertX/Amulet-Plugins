...
#Credits to PremiereHell for sharing with me the 1.19 INT id for java.
...
# Cave & Cliffs Part 2 #Added no loot tables.
# JAVA: ???? #Too lazy to grab.
# BEDROCK: (1,18,0)

# Cave & Cliffs Part 1 #Added no loot tables.
# JAVA: 2724
# BEDROCK: (1,17,0)
...
#updated to bedrock_1.19.0 & java_1.19
hardcoded_ops = [
...
class TabThree(wx.Panel):
    def __init__(self, parent):
        ...
        "So make sure the address you provide points to a valid loot table json in your behavior directory!\n"\
        "\n"\
...
class SetLootTables(wx.Panel, DefaultOperationUI):
    def __init__(
        ...
        self.loot_textbox.SetValue("loot_tables/directory_name/table_name.json")
    ...
    def _changed_text(self, evt):
        if self.loot_textbox.GetValue() == "loot_tables/directory_name/table_name.json":
            self.loot_textbox.SetStyle(0, 1000, wx.TextAttr((169, 169, 169, 255), (255, 255, 255, 128)))
        else:
    ...
    #downloads the current behavior pack zip from the stable version of MCBE if the temp directory doesn't exist.
    ...
    def _loot_table_strs(self, mc_v):
        ...
        #creates a temp directory for extracting loot tables from a link if local install fails
        #note this temp directory is removed when the operation is complete.
        self._get_temp_directory("temp")
    ...
    #gets a list of drive letters to for grabbing Minecraft Bedrocks data directory path later on.
    ...
    #creates a temp directory if it doesn't already exist.
    def _get_temp_directory(self, bk_type):
        try:
        ...
        if len(sel_block_entities) > 0:
            ...
            try:
                ...
                try:
                    ...
                    print ("Directory 'temp' was already removed sucessfully.")
                    pass
                pass
