...
class DumpCoordinates(wx.Panel, DefaultOperationUI):
    ...
    def _block_info(self, bk_array, bk_count, bk_type, ckey):
        ...
        if bk_type != "all":
            if bk_type == "counts":
                ...
                print ("(Dumped the following blocks at plugins/"+str(bk_type)+str(ckey)+"/<name>."+str(bk_type)+")")
                return self._dump_message(bk_array, bk_count)
        else:
            print ("(Dumped the following blocks to these paths)")
    ...
    def _get_folder(self, bk_type, bk_key):
        try:
            os.makedirs('plugins/'+str(bk_type)+str(bk_key))
        except:
            pass
    ...
    def _dump_coordinates(self):
        ...
        coords_path = [str(os.getcwd())+'\\plugins\\txt_coords',str(os.getcwd())+'\\plugins\\json_coords',str(os.getcwd())+'\\plugins\\json_counts',str(os.getcwd())+'\\plugins\\txt_counts']
        ...
        if operation_mode == "Coords to *."+str(jn):
            ...
            for bk in blocks_array:
               with open("plugins/"+str(jn)+str(ckey)+"/"+str(bk)+"."+str(jn),"w+") as jsonfile:
        ...
        #gets the selection mode of coords to *.txt
            ...
            for bk in blocks_array:
                with open("plugins/"+str(tx)+"_"+str(ckey)+"/"+str(bk)+"."+str(tx),"w+") as textfile:
        ...
        #gets the selection mode of coords to *.*
            ...
            for bk in blocks_array:
                with open("plugins/txt_coords/"+str(bk)+"."+str(tx),"w+") as textfile:
                ...
                with open("plugins/json_coords/"+str(bk)+"."+str(jn),"w+") as jsonfile:
                    json.dump({bk:blocks_array[bk]}, jsonfile, indent=2)
        ...
        #gets the selection mode of block counts to *.json
            ...
            with open("plugins/"+str(jn)+str(cckey)+"/block_counts."+str(jn),"w+") as jsonfile:
                countdict = {}
                countdict["info"] = {}
                countdict["blocks"] = {}
                for bkc in counts:
        ...
        #gets the selection mode of block counts to *.json
            ...
            with open("plugins/"+str(tx)+str(cckey)+"/block_counts."+str(tx),"w+") as textfile:
                cck = 0
                cck2 = 0
